...
//!
//! The `Document` struct is always available (chunkers consume it). The
//! file/HTTP/S3/PG fetcher implementations require the `source` Cargo feature
//! since they pull `reqwest`, `object_store`, `tokio`, `sqlx`, etc.
...
mod fetchers {
    ...
    use super::Document;
    use crate::config::{
        FilesSourceConfig, HttpSourceConfig, JsonCorpusSourceConfig, S3SourceConfig,
    };
...
// `PgTableSource` lives in its own gated module — it requires `sqlx`, which is
// only pulled in when the `sink` feature is on.
...
#[cfg(feature = "source")]
pub use fetchers::{FilesSource, HttpSource, JsonCorpusSource, S3Source};
