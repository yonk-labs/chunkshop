...
(multi-lang tree-sitter — Python, Java, Go, TypeScript, JavaScript, Rust, C,
C++, C#, Ruby — 10 languages, all via real grammars in the `[code]` extra;
`regex_fallback.py` is the safety net when
...
**RM-A — Rust port of SP-A** (chunkshop#9) **shipped 2026-05-19** and was merged into `main` (`--no-ff`). Spec retained at `docs/superpowers/specs/2026-05-19-chunkshop-rm-a-rust-memory-primitives-design.md`; plan archived at `archive/docs/superpowers/plans/2026-05-19-chunkshop-rm-a-rust-memory-primitives.md`. Latent correctness requirements (O1 session-level WHERE, O3 crash-safety, bi-temporal type discipline) were baked in from day 1 so the Python data-loss bug (`49861dc`) cannot recur in Rust.

**RM-B — Rust parity for SP-1 sync primitives + RawStore** shipped 2026-05-25 and was merged into `main` (`--no-ff`, 8/8 tasks, 40 new Rust tests, tag `rm-b-rust-sp1-parity`). Closes the v0.6.0 Python-vs-Rust behavioral gap for `SyncMode` / `IncrementalSource` / `PrunableSource` / `StaleCursorError` / `Document.fingerprint`, the `pg_table` tuple cursor (boundary-row safety), the `s3` ETag IncrementalSource, the `http` depth-crawl + ETag/Last-Modified cursor + robots.txt, and the `RawStore` primitive (filesystem + S3). Plan archived at `archive/docs/superpowers/plans/2026-05-25-rm-b-rust-sp1-parity.md`; handoff doc archived alongside.

Other in-flight plans live in `docs/superpowers/plans/`: lede v0.4 hints, search product, SP-1/SP-2/SP-3 connector + file-parser efforts, symbol-aware code ingest. Check `git log` and `git worktree list` to assess current status before assuming any of them are still active vs. shipped-but-not-archived.
