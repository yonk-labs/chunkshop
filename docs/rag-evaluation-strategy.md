...
For the current KB/profile direction from Phase D, see
[`phase-d-findings-next-steps.md`](phase-d-findings-next-steps.md) and
[`kb-profile-ladder-plan.md`](kb-profile-ladder-plan.md). The short version is
that different corpus shapes should normally be separate KBs/namespaces/tables,
because the winning retrieval and context-packing defaults are corpus-specific.
...
Treat a KB/namespace as a retrieval contract, not a random bucket of rows. The
contract includes chunking, metadata promotion, FTS fields, vector metric,
retrieval mode, context packer, default profile, and evaluation workload.

Split corpora into separate KBs when any of these differ:

- document shape: self-contained legal opinion vs cross-document news article
  vs code file;
- metadata: docket/term/citation vs source/date/entity vs path/symbol/language;
- expected question type: single-document explanation vs multi-hop synthesis vs
  deterministic metadata aggregation;
- context-packing winner: doc summary vs chunk summary plus raw evidence;
- token/latency budget;
- regression workload and success threshold.

Mixed KBs are allowed for convenience, but their defaults are compromises. If a
run includes mixed corpora, report per-corpus/per-source-family results in
addition to aggregate results. Aggregate pass rate alone can hide that a profile
improved one corpus while hurting another.
